...
void decryption(char texto_cifrado[], char cif_aux[], int n,double menor, Quadgrams *quadgrams) {
    ...
    for (int k = 1; k < 150; k++) {//loop responsavel em percorrer todo o alfabeto
        ...
        for (int x = 0; x + 4 <= tam; x++) {
            for (int k = 0; k < 4; k++) {
                pega_os_quad[k] = cifrado[x + k];
            }
            pega_os_quad[4] = '\0';
            float valor_quad = pontuar(pega_os_quad, quadgrams, n);
            soma += valor_quad;
        }
        pontuacao = soma;
        soma = 0.0;
        ...
        printf("\n\nTexto Descriptografado (chave %d): ", k);
        for(int l=0;l<tam;l++){
            printf("%c",cifrado[l]);
        }
        printf("\nPontuação: %f\n", pontuacao);
...
int main(){
    ...
    arquivo = fopen("arquivo_binario_substituicao.txt", "r");
